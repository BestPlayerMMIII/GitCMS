...
import type { GitCMSConfig, SchemaGroup, ContentItem, TransportMode, RateLimitInfo } from './types';
...
export class GitCMS {
  ...
  async doc(path: string): Promise<ContentItem | null> {
    if (this.transport === 'proxy' && this.config.baseUrl) {
      ...
      // Direct GitHub API access (public or authenticated)
      try {
        const [owner, repo] = this.config.repository.split('/');
