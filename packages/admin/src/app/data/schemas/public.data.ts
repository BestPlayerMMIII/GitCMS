 ...
 */
export async function schemasPublicGET(owner: string, repo: string, branch: string = 'main') {
  try {
    if (!owner || !repo) {
      throw new Error('Owner and repo parameters are required');
    }

    return await handleFetchPublicSchemas(owner, repo, branch);
  } catch (error) {
...
async function handleFetchPublicSchemas(owner: string, repo: string, branch: string) {
  try {
    ...
    throw new Error(
      error instanceof Error ? error.message : 'Failed to fetch schemas from repository'
    );
  }
}
